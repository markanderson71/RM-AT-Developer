...
export async function assemble(extraction, { sessionId = null, includeSelf = false, evidence = null, deps = {} } = {}) {
  ...
  const context = [
    ...
    exSlot.taken.length
      ? `## EXEMPLARS — sessions Chris scored himself, line by line: what Mark said, then Chris's number. These define the levels. For each line you score, find the exemplar line whose evidence most resembles this session's, start from Chris's number for it, and move off it only for a difference in the evidence that you can name. Cite the exemplar on every line it informed. An exemplar is evidence of level, not of content: never credit this session with something only the exemplar session said.\n\n${renderChunks(exSlot.taken)}`
