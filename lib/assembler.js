...
//     Empty until session 6; PSIA reference takes the slack up to its own cap.
import { embedQuery } from './embed.js';
import * as store from './store.js';
import { extractionToText } from './prompts/extract.js';
import { SKILLS } from './vocab.js';
 ...
 */
export async function assemble(extraction, { sessionId = null, includeSelf = false, deps = {} } = {}) {
  ...
  // Slot 2 — Chris. No skill filter: his general principles should surface (§7.3.1). Exemplars go to slot 3.
  const chrisRanked = (await db.searchByEmbedding(embedding, { authors: ['chris'], sources: CHRIS_SOURCES, limit: CHRIS_MAX_CHUNKS * 2 }))
  ...
  const exRanked = (await db.searchByEmbedding(embedding, { types: ['exemplar'], sources: ['chris_score'], limit: 12 })).filter(notSelf);
  ...
  const extractionText = renderExtraction(extraction);
  ...
  for (const [slot, s] of Object.entries(slots)) {
    for (const c of s.taken) {
      chunkIndex[shortId(c.id)] = {
        ...
        title: c.metadata?.title || null, similarity: c.similarity != null ? Number(c.similarity.toFixed(4)) : null,
        text: c.text,
      };
    }
  }
  ...
  const context = [
    ...
    exSlot.taken.length
      ? `## EXEMPLARS — sessions Chris scored himself. Match his level for comparable work.\n\n${renderChunks(exSlot.taken)}`
  ...
  const manifest = {
    ...
    chunks: Object.fromEntries(Object.entries(chunkIndex).map(([k, v]) => [k, { slot: v.slot, source: v.source, author: v.author, date: v.date, source_ref: v.source_ref, similarity: v.similarity }])),
  };
