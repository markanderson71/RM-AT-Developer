...
export function parseAIJson(resp) {
  ...
  try {
    ...
    if (m) {
      ...
      // 2026 lines too (session 5): a v2 summary that only survives by regex must not lose the rationale for the lines it is scored on.
      if (rm) for (const k of [...CRITERIA_KEYS, 'desired_performances', 'equipment']) { const v = rm[1].match(new RegExp(`"${k}"\\s*:\\s*"([^"]*)"`)); if (v) rationale[k] = v[1]; }
      return {
