...
export function comparisonFacts(c) {
  ...
  const one = (v) => {
    ...
    return { state, reference_quote: has(v.reference_quote) ? v.reference_quote : null, quote: has(v.quote) ? v.quote : null, prompted: v.prompted === true, ...(v.reference_is_marks_words === false ? { reference_is_marks_words: false } : {}) };
  };
