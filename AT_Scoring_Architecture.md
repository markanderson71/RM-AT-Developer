...
**Status:** Building — sessions 1, 3, 4 done; 2 waiting on a Zoom transcript; next buildable is 5
**Owner:** Mark Anderson
...
- **Coaching from `gap_to_next` (Mark, 2026-09-19: "one addition I like that I didn't plan for").** The per-line "Instead of X, say Y, because Z" rewrites are already produced by the evaluator and shown inside the collapsed rationale on the AT Exam score screen. Promote to a first-class feature: (1) session 5 — a Coaching panel at the top of each session in MA History: the six rewrites, Mark's original sentence beside each, cited Chris chunk underneath; fix the "Based on:" line (entries run together, untitled chunks show only a date); label rewrites as AI suggestions, not Chris's words. (2) session 5 or 7 — "Try that line again": Mark rewrites one passage, extract+evaluate re-runs on that line only (cheap, fast), shows whether the unit is now complete. (3) session 8 — Progress tab: recurring gaps across sessions ("how not stated" on 5 of last 6), which is what the rewrites have in common. (4) AT Exam setup — a three-line brief from the last session's top gaps before starting a new exam. Also route rewrites to Chris: his reactions to them are correction chunks (§6.1) and the fastest way to calibrate the coaching, not just the numbers.
...
- **Evaluate is unstable, not biased:** −1 on 7n6ry6d, +2 on 7uk7lnh, both driven by the single boolean `comparison_to_intended_outcome.made`. Chris's verbal reasoning on the 9/18 call (session 2 will extract it) should define what he counts as the comparison — likely the task's ideal, not only the peer's stated intent.
