...
- **`api/claude.js` has no heartbeat.** Peer/examiner turns are short; the legacy-scorer fallback is the only long call through it and now has 300 s. If the fallback is ever seen failing with "Unable to reach" again, give it the same `slow()` wrapper.
