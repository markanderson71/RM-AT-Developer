// /api/sheet for the OLD app (at-dev-tracker) — session 7b. Self-contained copy of lib/ops.js + api/sheet.js from
...
const TABLE = 'ops_rows';
const SHEETS = ['Config', 'Journal', 'MASessions'];
...
async function getAll(sheet, db = supabaseAdapter) {
  checkSheet(sheet);
...
async function update(sheet, data, db = supabaseAdapter) {
  checkSheet(sheet);
...
async function create(sheet, data, db = supabaseAdapter) {
...
async function remove(sheet, id, db = supabaseAdapter) {
  checkSheet(sheet);
...
function recipientsFrom(profilesJson) {
  let profiles = {};
...
async function notify(data, db = supabaseAdapter, { send = sendResend } = {}) {
...
async function handle(body, db = supabaseAdapter, opts = {}) {
...
export default async function handler(req, res) {
  ...
  if (out?.transient) { console.warn(`sheet: ${body._action} ${body._sheet} → store error: ${out.error}`); return res.status(502).json({ error: out.error }); }
  if (Date.now() - t0 > 2000) console.warn(`sheet: ${body._action} ${body._sheet} took ${Date.now() - t0} ms`);
