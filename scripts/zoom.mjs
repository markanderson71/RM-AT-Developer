...
const OPTS = ['--date', '--map', '--title', '--mentor', '--pending'];
...
const r = await ingestZoom({ transcript: readFileSync(file, 'utf8'), date: opt('--date'), title: opt('--title'), mentor: opt('--mentor') || 'chris', speakerMap, dryRun: flag('--dry'), replacePending: flag('--replace'), onStep: (m) => console.log(`  … ${m}`) });
...
r.rejected.forEach((x) => console.log(`  REJECTED [${x.why}] ${x.timestamp || ''} ${x.text}`));
