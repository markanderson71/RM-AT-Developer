...
const compare = opt('--compare')?.split(',').map(Number) || null;
...
for (const id of ids) {
  ...
  console.log(`  context ${m.tokens.total} tok · chris ${m.counts.chris} · exemplars ${m.counts.exemplars} · psia ${m.counts.psia} · ${Math.round(r.meta.ms.total / 1000)}s`);
